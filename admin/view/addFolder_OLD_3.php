...
<script>
  (function(folders){
    ...
    $("#addButton").click(function(){
        if($('#nameOfCategory').val() !== ''){
            $.ajax({url: "<?=$file_padder?>/utils/addCategory.php", data: $("form").serialize(), success: function(result){               
                ...
                var folderList = JSON.parse(result);
                populateSelects(folderList);
                //Clearing Form
                $('#nameOfCategory').val('');
                $('#urlOfCategory').val('');
            }});
    ...
    $('#edit-page').click(()=>{
        if($('#editName').val() !== ''){
            $.getJSON("<?=$file_padder?>/utils/editFolder.php", {title: $('#editName').val(), url: $('#editURL').val(), id : $('#editSelector').val()}, (result)=>{
                console.log(result);
            })
        }
    })
